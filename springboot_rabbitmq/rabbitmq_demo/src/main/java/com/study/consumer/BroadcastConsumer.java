...
public class BroadcastConsumer {
    private Logger logger= LoggerFactory.getLogger(BroadcastConsumer.class);

    @RabbitHandler
    public void onmessage(BroadcastMessage message){
        logger.info("[BroadcastConsumer.onmessage][线程编号:{},消息内容:{}]",Thread.currentThread().getId(),message);
    }
}
